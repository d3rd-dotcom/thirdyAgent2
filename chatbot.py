...
# Load .env before anything else so os.environ has all keys.
# Must run before telegram.notifier import — notifier reads os.environ at thread start.
...
try:
    ...
    print("  [TELEGRAM] telegram/notifier.py not found — skipping")

# ─────────────────────────────────────────────────────────────────────
#  CONFIGURATION
# ─────────────────────────────────────────────────────────────────────

# CRIT-01 FIX: All credentials loaded from config.py — never hardcode here
from config import (
    AGENTHUB_API_KEY as API_KEY,
    AGENT_ID,
    PUBLIC_URL as NGROK_URL,
...
def save_replied(msg_id):
    # HIGH-01 FIX: prune replied_ids.txt to last 10,000 entries to prevent OOM
    try:
...
def instant_crypto(text=""):
    ...
    # EFF-01 FIX: route through agent.py /webhook to use the 60s CoinGecko cache
    # avoids double API calls when chatbot and agent both query CoinGecko
    try:
        r = requests.post(
            "http://localhost:5000/webhook",
            json={"skill": "crypto", "parameters": {"coin": coin}},
        ...
        pass
    # Fallback: direct CoinGecko if agent.py is not running
    try:
        r = requests.get(
...
""")
    ...
    if _TELEGRAM_READY:
        start_notifier()
        print("  [TELEGRAM] Notifier thread started")
