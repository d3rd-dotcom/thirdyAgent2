...
def chat_webhook():
    # HIGH-03 FIX: reject requests that don't carry the AgentHub Authorization header
    # AgentHub always sends Authorization: Bearer <AGENTHUB_API_KEY> on legitimate calls
    ...
    if send_instant_reply(from_id, reply):
        if msg_id:
            with _CHAT_REPLIED_LOCK:
                ...
                # HIGH-04 FIX: prune in-memory set to prevent unbounded growth
                if len(CHAT_REPLIED) > 50_000:
                    CHAT_REPLIED.clear()
